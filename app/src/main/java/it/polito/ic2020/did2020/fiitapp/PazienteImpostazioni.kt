...
import kotlinx.android.synthetic.main.paziente_impostazioni.*


class PazienteImpostazioni: Fragment(R.layout.paziente_impostazioni) {

    val user = Firebase.auth.currentUser
    val fDatabase = FirebaseFirestore.getInstance()
    val dbUsers = fDatabase.collection("users").document(user.uid)
    val dbPaziente= fDatabase.collection("pazienti").document(user.uid)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        logout.setOnClickListener{
            ...
            findNavController().navigate(R.id.action_pazienteImpostazioni_to_frgmt_main)
        }
        ...
        delete.setOnClickListener{
            ...
            }).setNeutralButton("Annulla",{ dialogBuilder, which -> dialogBuilder.dismiss() })
        ...
        SalvaM.setOnClickListener{
            if(MailEdit.text != null && MailEdit.text.toString().isNotEmpty()){
